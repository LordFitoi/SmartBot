...
class MarkovModel:
    ...
    def __call__(self, initial_token="", text_lenght=20) -> str:
        if self.memory:
            word_list = ["START"]
            if initial_token:
                word_list.append(initial_token)
...
class ContextMarkovModel:
    ...
    def train(self, sent_list: list) -> None:
        for i in range(len(sent_list) - 1):
            if not sent_list[i + 1] or not sent_list[i]:
                continue
        ...
        if self.memory:
            ...
            for _ in range(text_lenght):
                if current_token not in self.memory:
                    if current_token[1]:
                        candidates = [
                            item for item in self.memory if item[1] == current_token[1]
                        ]

                    else:
                        candidates = [
                            item for item in self.memory if item[0] == current_token[0]
                        ]
                    if not candidates:
                        candidates = list(self.memory.keys())
                ...
                for word, value in self.memory[current_token].items():
                    if random.randrange(0, max_freq + 1) <= value:
                        ...
                        if index >= len(context):
                            ...
                            x_input = context[index]

                        word_list.append((word, x_input))
                        break
                    else:
                        max_freq -= value

                if word_list[-1][0] == "END":
                    break
                current_token = word_list[-1]
